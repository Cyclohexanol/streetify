...
The game provides a standard render of the entities shown below. However, the render and "backend" are totally separated allowing the community to customize the skin of the game.
...
There are three type of items personal, floor and wall. All items can be bought, sold, stored and are crafted wither by a type of craftman or later on machines. The craftman's skill or the machine performance determines the quality of that item which affects the effectiveness. Items can get unlocked through research (see research section).
...
Animals can be wild or domesticated. They can come from the wild or breed.
